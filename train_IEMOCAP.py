...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, modals, optimizer=None, train=False, dataset='IEMOCAP'):
	...
	for data in dataloader:
		...
		loss = loss_function(log_prob, label)+0.01*(target_criterion(wt1[:,-1:],s_t)+target_criterion(wt2[:,-1:],s_t)+target_criterion(wt3[:,-1:],s_t)+target_criterion(wt4[:,-1:],s_t)+target_criterion(wa[:,-1:],s_t)+target_criterion(wv[:,-1:],s_t))
		...
		for i in range(len(umask)):
			for j in range(len(umask[i])):
				if umask[i][j] == 0:
					break
				loss = loss + 0.8*0.0001*(target_criterion(pt1[now][:-1].unsqueeze(1),ss_t)+target_criterion(pt2[now][:-1].unsqueeze(1),ss_t)+target_criterion(pt3[now][:-1].unsqueeze(1),ss_t)+target_criterion(pt4[now][:-1].unsqueeze(1),ss_t)+target_criterion(pa[now][:-1].unsqueeze(1),ss_t)+target_criterion(pv[now][:-1].unsqueeze(1),ss_t))
				now += 1
...
if __name__ == '__main__':
	...
	for e in range(n_epochs):
		...
		if args.graph_model:
			...
			test_loss, test_acc, test_label, test_pred, test_fscore, _, _, _, _, _, test_probs = train_or_eval_graph_model(model, loss_function, test_loader, e, cuda, args.modals, dataset=args.Dataset)
